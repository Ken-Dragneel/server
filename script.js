...
// Wait until the DOM is loaded before adding event listeners
    ...
    const details = document.querySelectorAll('.detail-info');

    // Function to handle the click event
    function showDetails(event) {
        // Get the clicked founder's data-id attribute
        const founderId = event.target.closest('.founder').getAttribute('data-id');
        ...
        // Show the clicked founder's details with an animation
        const detailToShow = document.getElementById(`detail-${founderId}`);
        if (detailToShow) {
            detailToShow.style.display = 'block';
        }
    }

    // Add event listeners to all the founder names
    founders.forEach(founder => {
        founder.addEventListener('click', showDetails);
    });
});
